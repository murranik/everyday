          ...
          Container(
            ...
            child: SfCalendar(
              ...
              appointmentBuilder: (BuildContext context,
                ...
                final AppointmentExtend meeting =
                    details.appointments.first;

                return  GestureDetector(
                    onTap: () async {
                  var event = await DBProvider.db.getModelById(meeting.eventId, Event(label: ""));
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: Text(
                                meeting.subject +
                                    (intl.DateFormat.yMd().format(meeting.startTime) != intl.DateFormat.yMd().format(meeting.endTime)
                            ...
                            Expanded(
                              ...
                              flex: intl.DateFormat.yMd().format(meeting.startTime) == intl.DateFormat.yMd().format(meeting.endTime) ? 0 : 2,
                            )
                          ],
                        ),
                        if(intl.DateFormat.yMd().format(meeting.startTime) == intl.DateFormat.yMd().format(meeting.endTime))
                          Row(
                            children: [
                              Text(
                                intl.DateFormat.Hm().format(meeting.startTime) == intl.DateFormat.Hm().format(meeting.endTime) ? "В " + intl.DateFormat.Hm().format(meeting.startTime) : intl.DateFormat.Hm().format(meeting.startTime) + " - " + intl.DateFormat.Hm().format(meeting.endTime),
                                overflow: TextOverflow.ellipsis,
                              ),
                            ],
                          )
...
class AppointmentExtend extends Appointment{
  ...
  }) : super(startTime: extendStartTime, endTime: extendEndTime, color: extendColor,subject: extendSubject);
}
