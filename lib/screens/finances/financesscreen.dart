...
class _FinancesScreenState extends State<FinancesScreen> {
  ...
  Future<List<dynamic>> getFinances() async {
    var finances = await DBProvider.db
        .getModels(FinanceModel(price: 0.0, isIncome: 0)) as List<FinanceModel>;
    incomeFinances =
        finances.where((element) => element.isIncome == 1).toList();
    notIncomeFinances =
        finances.where((element) => element.isIncome == 0).toList();
  ...
  void addFinanceModelToList(FinanceModel financeModel,
    ...
    if (!toDelete) {
      if (financeModel.isIncome == 0) {
        notIncomeFinances
            .removeWhere((element) => element.id == financeModel.id);
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            var dataListForNotIncomeFinances = notIncomeFinances.length > 3
                ? notIncomeFinances
                    .sublist(
                        notIncomeFinances.length - 3, notIncomeFinances.length)
                    .reversed
                    .toList()
                : notIncomeFinances;
            var dataListForIncomeFinances = incomeFinances.length > 3
                ? incomeFinances
                    .sublist(incomeFinances.length - 3, incomeFinances.length)
                    .reversed
                    .toList()
                : incomeFinances;
            return Container(
                                  ...
                                  itemBuilder: (context, index) {
                                    return GestureDetector(
                                            ...
                                            builder: (builder) {
                                              return Container(
                                                  ...
                                                  child: Container(
                                                    ...
                                                    child: FinanceDialog(
                                                      financeModel:
                                                          dataListForIncomeFinances[
                                                              index],
                                                      update:
                                                          addFinanceModelToList,
                                                    ),
                                                  ));
                                            });
                                      },
                                      child: Row(
                                        ...
                                        children: [
                                          Text(
                                            dataListForIncomeFinances[index]
                                                    .label ??
                                                "Без назви",
                          ...
                          children: [
                            if (dataListForNotIncomeFinances.isNotEmpty)
                              ListView.builder(
                                  shrinkWrap: true,
                                  itemCount:
                                      dataListForNotIncomeFinances.length,
                                  physics: const NeverScrollableScrollPhysics(),
                                  itemBuilder: (context, index) {
                                    return GestureDetector(
                                        onTap: () {
                        ...
                        DecoratedContainer(
                          ...
                          children: [
                            Builder(builder: (context) {
                              var income = incomeFinances.fold(
                                  0, (previousValue, element) => element.price);
                              var notIncome = notIncomeFinances.fold(
                                  0, (previousValue, element) => element.price);
                              return Column(
                                children: [
                                  Text(
                                    "Дохід = ${income.toStringAsFixed(2)}. Витрати = ${notIncome.toStringAsFixed(2)} ",
                                    style: TextStyle(fontSize: 12.sp),
                                  ),
                                  Text(
                                    "Баланс = ${(budget + income - notIncome).toStringAsFixed(2)} ",
                                    style: TextStyle(fontSize: 12.sp),
                                  ),
                                ],
                              );
